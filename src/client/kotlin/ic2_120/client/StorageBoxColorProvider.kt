...
object StorageBoxColorProvider {
    ...
    fun register() {
        val provider = BlockColorProvider { state, world, pos, tintIndex ->
        ...
        for (blockId in storageBoxIds) {
            ...
            if (block.isPresent) {
                ColorProviderRegistry.BLOCK.register(provider, block.get())
                registeredCount++
            }
        }

        logger.info("已注册 {} 个储物箱着色器", registeredCount)
    }
}
