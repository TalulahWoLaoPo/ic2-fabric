...
class ExtractorScreen(
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        val progressFrac = if (ExtractorSync.PROGRESS_MAX > 0) {
            (handler.sync.progress.coerceIn(0, ExtractorSync.PROGRESS_MAX)
                .toFloat() / ExtractorSync.PROGRESS_MAX).coerceIn(0f, 1f)
        } else 0f
            ...
            Row(
                ...
                Column(
                    ...
                    Flex(
                        ...
                        Column(spacing = 4) {
                            SlotHost(ExtractorScreenHandler.SLOT_INPUT_INDEX)
                            SlotHost(ExtractorScreenHandler.SLOT_DISCHARGING_INDEX)
                        }
                        EnergyBar(progressFrac, modifier = Modifier.EMPTY.fractionWidth(1.0f))
                        SlotHost(ExtractorScreenHandler.SLOT_OUTPUT_INDEX)
                    }
                }
                ...
                Column(
                    ...
                    for (slotIndex in ExtractorScreenHandler.SLOT_UPGRADE_INDEX_START..ExtractorScreenHandler.SLOT_UPGRADE_INDEX_END) {
                        SlotHost(slotIndex)
                    }
                }
            }
    ...
    private fun UiScope.SlotHost(slotIndex: Int) {
        SlotAnchor(
            ...
            width = ExtractorScreenHandler.SLOT_SIZE,
            height = ExtractorScreenHandler.SLOT_SIZE
        )
    }
