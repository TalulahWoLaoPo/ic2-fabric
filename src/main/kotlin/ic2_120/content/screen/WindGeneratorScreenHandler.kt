...
@ModScreenHandler(block = WindGeneratorBlock::class)
class WindGeneratorScreenHandler(
    syncId: Int,
    playerInventory: PlayerInventory,
    blockInventory: Inventory,
    private val context: ScreenHandlerContext,
    private val propertyDelegate: PropertyDelegate
) : ScreenHandler(WindGeneratorScreenHandler::class.type(), syncId) {
    ...
    init {
        ...
        addSlot(PredicateSlot(blockInventory, WindGeneratorBlockEntity.BATTERY_SLOT, BATTERY_SLOT_X, BLOCK_SLOTS_Y, BATTERY_SLOT_SPEC))
    ...
    override fun quickMove(player: PlayerEntity, index: Int): ItemStack {
        ...
        if (slot.hasStack()) {
            ...
            when {
                index == WindGeneratorBlockEntity.BATTERY_SLOT -> if (!insertItem(stackInSlot, 1, 38, true)) return ItemStack.EMPTY
                index in 1..37 -> {
                    val moved = SlotMoveHelper.insertIntoTargets(
                        stackInSlot,
                        listOf(SlotTarget(slots[WindGeneratorBlockEntity.BATTERY_SLOT], BATTERY_SLOT_SPEC))
                    )
    ...
    override fun canUse(player: PlayerEntity): Boolean =
        context.get({ world, pos ->
            world.getBlockState(pos).block is WindGeneratorBlock && player.squaredDistanceTo(
    ...
    companion object {
        ...
        fun fromBuffer(syncId: Int, playerInventory: PlayerInventory, buf: PacketByteBuf): WindGeneratorScreenHandler {
            ...
            return WindGeneratorScreenHandler(syncId, playerInventory, blockInv, context, ArrayPropertyDelegate(propertyCount))
        }
    }
}
